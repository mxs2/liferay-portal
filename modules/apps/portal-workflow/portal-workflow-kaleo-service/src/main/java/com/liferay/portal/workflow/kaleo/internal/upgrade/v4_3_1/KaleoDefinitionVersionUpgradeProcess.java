...
public class KaleoDefinitionVersionUpgradeProcess extends UpgradeProcess {
	...
	protected void doUpgrade() throws Exception {
		_upgradeKaleoDefinitionVersion(
			"kaleoDefinitionVersionId", "KaleoDefinitionVersion");
	}
		...
		try (PreparedStatement preparedStatement1 = connection.prepareStatement(
				SQLTransformer.transform(
					StringBundler.concat(
						...
						", name from ", tableName,
						"where name = 'message-boards-user-stats-moderation' ",
						"or name = '", WorkflowDefinitionConstants.
							NAME_SINGLE_APPROVER, "'")));
			 PreparedStatement preparedStatement2 =
				 AutoBatchPreparedStatementUtil.concurrentAutoBatch(
					 connection,
					 StringBundler.concat(
						 "update ", tableName, " set name = ? where ",
						 "ctCollectionId = ? and ", columnName, " = ?"));
